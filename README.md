...
**Salary is the headline.** Twelve monthly payroll runs of $6,050–$7,400 that add up to exactly **$80,000 net over the year**, paid on the 5th by Northwind Digital Ltd — plus quarterly bonuses, a 13th salary, expense reimbursements. Every salary credit carries a real payslip (gross, income tax, social insurance, net) and reads as payroll everywhere it appears: a green lane and a SALARY badge in the list, a payroll card on Home with the next pay date and twelve bars, and a full payslip inside the receipt. Other income sits beside it: marketplace payouts, contract milestones, dividends, a tax refund, cashback, referral bonuses, card refunds, vault interest and inbound peer transfers.
...
**~1,270 transactions across thirteen months in 34 categories**, with three statuses that behave differently: completed, pending (an authorisation hold leaves the available balance immediately, an incoming payment does not) and declined (struck through, never touches the balance, with the reason attached).
...
**Receipts.** Every row in the app — salary, card payment, transfer, exchange, ATM, declined attempt — opens a full receipt: paper-style slip with a torn edge, an angled status stamp, the amount, both parties with account numbers, method, card, fee, FX rate, balance after, a barcode and a verification code, plus actions (repeat the payment, download the payslip, copy the code, open that month's statement, dispute). Salary receipts swap the middle section for the payslip breakdown. It prints to PDF cleanly.

**Periods.** Activity opens on the **last 7 days** — the week you actually care about. From there it switches to 1M / 3M / 6M / 1Y / All — totals, charts, categories and merchants all recompute. Activity also filters by type (income, card, subscriptions, transfers, cash, exchange, pending, declined), searches by merchant, person, reference or amount, and pages through the ledger 80 rows at a time. Reaching back six months or more pulls hundreds of rows, so those periods show a shimmering skeleton while the history loads. Tapping a category bar in Analytics opens every payment inside it.
